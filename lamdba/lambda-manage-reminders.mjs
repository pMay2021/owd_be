...
export const handler = async (event, context) => {
  ...
  if (!command) {
    return owd.Response(400, "Missing/Invalid command");
  }
  ...
  try {
    if (command === "add") {
      ...
      if (!customerDb) {
        return owd.Response(404, { message: "Customer not found", email: email });
      }
      ...
      if (!offsetArray || offsetArray.length === 0) {
        throw new Error("Invalid reminderOffsetDays");
      }
    ...
    if (command === "delete") {
      ...
      if (!id) {
        return owd.Response(400, "Missing/Invalid id");
      }

      const noticeDb = await db.getItem("db-notices", id);
      if (!noticeDb) {
        return owd.Response(404, { message: "Notice not found", id: id });
      }
    ...
    if (command === "update") {
      const { id } = event.queryStringParameters;
      if (!id) {
        return owd.Response(400, { message: "Missing/Invalid id", id: id });
      }

      const noticeDb = await db.getItem("db-notices", id);
      if (!noticeDb) {
        return owd.Response(404, { message: "Missing/Invalid notice", id: id });
      }
