...
const getVersion = () => {
  return {
    ...
    changes: "Now with encryption and decryption functions.",
  };
};
...
const encryptWithExpiry = (email, currentTime, secretKey) => {
  const expiryTime = new Date(new Date(currentTime).getTime() + 15 * 60 * 1000); // Add 15 minutes to current time
  const data = email + '|' + expiryTime.toISOString(); // Concatenate email and expiry time
  const iv = crypto.randomBytes(16).toString('hex'); // Generate a valid initialization vector
  const cipher = crypto.createCipheriv('aes-256-cbc',secretKey, iv); // Use AES-256-CBC algorithm with a secret key and the generated initialization vector
  let encrypted = cipher.update(data, 'utf8', 'hex');
  encrypted += cipher.final('hex');
  return encrypted;
};

const decryptWithExpiry = (encryptedData, secretKey) => {
  const decipher = crypto.createDecipheriv('aes-256-cbc', secretKey); // Use the same algorithm and secret key for decryption
  let decrypted = decipher.update(encryptedData, 'hex', 'utf8');
  decrypted += decipher.final('utf8');
  const [email, expiryTimeString] = decrypted.split('|'); // Split decrypted data into email and expiry time
