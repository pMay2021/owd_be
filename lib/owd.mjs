...
const getVersion = () => {
  return {
    ...
    changes: "getFriendlyDateDifference now returns an object with years, months, and days so frontend can use it to display the difference in a more flexible way.",
  };
};
...
const validateEmail = (email) => {
  email = email.toloLowerCase().trim();
  const re = /\S+@\S+\.\S+/;
  return re.test(email);
}

export { getDayOfWeek, getShortId, getOffsetDates, getVersion, getNumberOfDaysBetweenDates, getFriendlyDateDifference, log };
