...
const getVersion = () => {
  return {
    ...
    changes: "minor date enhancement"
  };
};
...
const httpStatusCodes = {
  ...
  102: 'Processing (WebDAV)', // WebDAV feature
  103: 'Early Hints',
  ...
  205: 'Reset Content',
  206: 'Partial Content',
  207: 'Multi-Status (WebDAV)', // WebDAV feature
  ...
  422: 'Unprocessable Entity (WebDAV)', // WebDAV feature
  423: 'Locked (WebDAV)', // WebDAV feature
  424: 'Failed Dependency (WebDAV)', // WebDAV feature
  ...
  506: 'Variant Also Negotiates (RFC 2116)',
...
const getOffsetDates = (expiryDate, offsetDays) => {
  ...
  let completeOffsetDays = [...new Set([...offsetDays, "0", "1", "7"])];
  
  //order completeOffsetDays ascending
  completeOffsetDays.sort((a, b) => a - b); 
  ...
  let newDates = completeOffsetDays.map((days) => {
    ...
    const theDate = newDate.toISOString().slice(0, 10);

    return {
      date: theDate,
      dayOfWeek: dayOfWeek,
      dueInDays: days,
      longDate: getFriendlyDate(theDate),
      display: getFriendlyDateDifference(theDate, expiryDate),
    };
  });
 ...
 * @param {object} obj - Detail object
 * @returns {object} The JSON response object.
 */
function Response(code, obj = null) {
  const body = {
    title: httpStatusCodes[code] || "Unknown Error",
    type: "/support/codes/" + code,
    status: code,
    detail: obj ?? {message: httpStatusCodes[code]}
  };
