...
const getVersion = () => {
  return {
    ...
    logLevel: logLevel,
    date: "2024-04-12",
    details: "minor updates, added node-fetch; lib with multitude of reusable functions, nanoid, marked, and more.",
    changes: "Disposable Email check, fetchURL",
  };
};
 ...
 * @param {boolean} [short=true] - Whether to use the short format (e.g., "Fri, 12th Feb, 2025").
 * @returns {string} The date in the friendly format.
 */
function getFriendlyDate(date, short = true, format = "") {
  const daysOfWeek = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
  const daysOfWeekShort = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
  const d = new Date(date);
  const dayOfWeek = short ? daysOfWeekShort[d.getDay()] : daysOfWeek[d.getDay()];
  const day = d.getDate();
  const month = months[d.getMonth()];
  const year = d.getFullYear();
  const suffix = getNumberSuffix(day);
  if (format === "t1") return ` ${month} ${day}${suffix} (${dayOfWeek}), ${year}`;
...
const getShortId = (numberOfDigits = 15, customDigits = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-") => {
  ...
  const nanoid = customAlphabet(customDigits, defaultLength);
  const id = nanoid(numberOfDigits > defaultLength || numberOfDigits < 3 ? defaultLength : numberOfDigits);
  return id;
};
...
const isValidEmail = (email) => {
  ...
  const re = /^(?=.{1,256})(?=.{1,64}@.{1,255}$)[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/;
...
/**
 * Fetches data from the specified URL using the provided method, body, and token.
 ...
 */
async function fetchURL(url, method, body, token) {
  let response;
