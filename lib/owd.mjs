...
const getVersion = () => {
  return {
    ...
    changes: "removed console.log from a function"
  };
}
...
const addToDBNoticeTable = async (entry) => {
  try {
    ...
    console.log("Entry added to db-notice table successfully");
  } catch (error) {
    console.error("Error adding entry to db-notice table:", error);
    throw error;
  }
};
