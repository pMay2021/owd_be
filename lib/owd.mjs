...
const getVersion = () => {
  return {
    ...
    changes: "Now with improved debug logging based on levels  with JSON response wrapper and HTTP error codes",
  };
};
 ...
 * @param {boolean} [level="DEBUG"] - only shows if level = DEBUG (set to anything else to turn off)
 * @returns {void}
 */
