...
const getVersion = () => {
  return {
    version: "1.6.0",
    date: "2024-04-12",
    details: "minor update to catch errors on getS3Text",
    changes: "getNoticesBySlot"
  };
};
...
const getS3Text = async (bucketName, filePath) => {
  try {
    ...
    console.error(error);
    return "Error getting file from S3.: " + error.message;
  }
};
...
const getNoticesBySlot = async (slot, tableName = "db-notices") => {
  const query = `SELECT * FROM "${tableName}" WHERE "slot" = '${slot}'`;
  const items = await executeStatement(query);
  return items;
};
